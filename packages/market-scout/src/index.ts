...
async function runHttp() {
  ...
  httpServer.listen(port, () => {
    process.stderr.write(`aurais-mcp-market-scout v${PACKAGE_VERSION} started (http) on :${port}/mcp — key via X-Anthropic-Key header; ${OAUTH_ENABLED ? `OAuth resource server (issuer ${OAUTH_ISSUER}); ` : ""}serve behind HTTPS\n`);
  });
}
